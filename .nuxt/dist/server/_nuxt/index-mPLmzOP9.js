...
import { u as useAsyncData, H as Header1, a as Header1Mobile, V as VContainer, R as Render } from "./asyncData-BoxtDLvH.js";
import { F as FavAndbasketIcons } from "./favAndbasketIcons-C2ijvWaG.js";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const langParam = computed(() => config.public.defaultLang);
    const [
      { data: languages },
      { data: settingsData },
      { data: rowsData },
      ...
      useAsyncData(`settings-${langParam.value}`, () => $fetch(`/api/pages/get/title/${langParam.value}`, { baseURL: config.public.baseUrl })),
      useAsyncData(`rows-${langParam.value}`, () => $fetch(`/api/pages/rows/title/${langParam.value}`, { baseURL: config.public.baseUrl })),
    ...
    const locale = computed(() => config.public.defaultLang || langParam.value);
    ...
    const pageSettings = computed(() => ({
      ...
      margin_top: settingsData.value?.data?.margin_top ?? null,
...
//# sourceMappingURL=index-mPLmzOP9.js.map
